...
from random import random, gauss, expovariate, sample
import time
...
class Grafo:
    def __init__(self):
        ...
        self.color = dict()
        self.posicion = dict()
        self.status = dict() #s = 1, t = 2, ninguno = 0
    ...
    def contraccion(self):
        ...
        x = (x1+x2)/2
        y = (y1+y2)/2
        self.agrega(newnode,1,(x,y),0)
        self.quitar_arista(a,b)
        V = self.vecinos[a].union(self.vecinos[b])
        for i in V:
            peso = 0
            if i in self.vecinos[a]:
                peso += self.aristas[(a,i)][0]
            if i in self.vecinos[b]:
                peso += self.aristas[(b,i)][0]
            self.conecta(newnode, i, peso, False, 1)
        self.quitar_nodo(a)
        self.quitar_nodo(b)

    def contraccion_st(self):
        #self.status[s] = 2
        #self.status[t] = 3
        found = False
        while not found:
        ...
        y = (y1+y2)/2
        self.agrega(newnode,1,(x,y),0)
        self.status[newnode] = max(self.status[a],self.status[b])
        self.quitar_arista(a,b)
        V = self.vecinos[a].union(self.vecinos[b])
        for i in V:
            peso = 0
